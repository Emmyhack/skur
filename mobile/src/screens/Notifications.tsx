...
import { BackButton, Badge, Card, CircleIcon, Empty, Row, Screen, SectionLabel, TopBar, useStyles } from "../components/ui";
  ...
  return (
    <Screen padded={false} top={<TopBar left={<BackButton onPress={() => nav.goBack()} />} center={<Text style={s.topTitle}>Updates</Text>} />}>
      <View style={{ padding: 16 }}>
        <Card flush>
          <Row leading={<CircleIcon name="bell" size={36} tone="accent" />} title="Notify me on this phone" subtitle="When a proposal needs you" trailing={<Switch value={notifications} onValueChange={(v) => void setNotifications(v)} trackColor={{ true: C.accent }} />} last />
        </Card>
