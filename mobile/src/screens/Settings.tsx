  ...
  return (
    <Screen padded={false} top={<TopBar title="Settings" right={<Logo size={26} wordmark={false} />} />}>
      <View style={{ padding: 16 }}>
        ...
        <Card flush>
          {vaultAddress ? (
            <>
              ...
              <Row leading={<CircleIcon name="book" size={36} />} title="Address book" subtitle="Recipients, activation and trust" onPress={() => nav.navigate("AddressBook")} chevron />
              <Row leading={<CircleIcon name="activity" size={36} />} title="Simulator" subtitle="Run attacks against the policy" onPress={() => nav.navigate("Simulator")} chevron />
              <Row leading={<CircleIcon name="bell" size={36} />} title="Updates" subtitle="What needs you, recent activity" onPress={() => nav.navigate("Notifications")} chevron />
              ...
              <Row leading={<CircleIcon name="plus-square" size={36} />} title="Create a new vault" subtitle="Name, template, members, one signature" onPress={() => nav.navigate("CreateVault")} chevron last />
            </>
        ...
        <Card flush>
          <Row leading={signer ? <Identicon address={signer.address} size={40} /> : <CircleIcon name="key" size={40} tone="accent" />} title={signer ? signer.label : "No signer yet"} subtitle={signer ? <Address value={signer.address} /> : "Create or import a key to sign from this phone"} trailing={roles ? <Badge tone="accent">{roleNames(roles).join(" · ")}</Badge> : signer ? <Badge tone="neutral">no role here</Badge> : undefined} onPress={() => nav.navigate("Signers")} chevron />
          <Row leading={<CircleIcon name="smartphone" size={36} />} title="Require Face ID to sign" subtitle={bioAvailable ? "Prompted before every signature" : "No biometrics enrolled on this device"} trailing={<Switch value={biometrics} onValueChange={(v) => void setBiometrics(v)} trackColor={{ true: C.accent }} />} last />
        </Card>
